...
let maxVisiblePages = 20; // Máximo de números de página visibles
let visiblePageStart = 1; // Primer número visible en la paginación
...
//     clearTimeout(debounceTimer); // Cancela el temporizador anterior si el usuario sigue escribiendo
//     debounceTimer = setTimeout(() => {
//         // Inicia la búsqueda después de 700ms de inactividad
//         currentPage = 1;
//         visiblePageStart = 1;
//         const query = document.getElementById("search-input").value.trim();
//         const type = document.getElementById("search-type").value;
...
// function searchGitHub(query, type, page) {
//     const url = `https://api.github.com/search/${type}?q=${query}&per_page=${resultsPerPage}&page=${page}`;
...
// // Funciones para crear tarjetas (repos y usuarios)
// function createRepoCard(repo) {
//     const card = document.createElement("div");
//     card.classList.add("card", "card-git");
...
//             <p class="card-text">${repo.description || "No description available."}</p>
//             <div class="mt-3">
//                 <p class="card-text mb-0 mx-3"><i class="fa-solid fa-star"></i> ${repo.stargazers_count || "No stars"}</p>
//                 <hr class="my-2">
//                 <p class="card-text mb-0 mx-3"><i class="fa-solid fa-circle-exclamation"></i> ${repo.open_issues_count || "No issues"}</p>
//             </div>
//         </div>
//         <div class="card-footer text-body-secondary d-flex justify-content-center mare">
//             <a href="${repo.html_url}" target="_blank" class="card-link link-underline-light text-dark">Vai al repo <i class="fas fa-arrow-up-right-from-square"></i></a>
